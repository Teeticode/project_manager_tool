...
const filterContainer = document.querySelector(".portfolio-filter-inner"),
      ...
      btns = document.querySelectorAll(".btn-login"),
      login = document.getElementById("login"),
      btnr = document.getElementById("btn-signup"),
      btnl = document.getElementById("btn-login"),
      signup = document.getElementById("signup"),
      forms = document.querySelectorAll(".form"),
      btnheaders = document.querySelector(".btn-headers"),
      loginmodal = document.querySelector(".login-modal .card"),
      authbtn = btnheaders.children
      totalbtns = authbtn.length;
      login.classList.add("hide")
        btnl.addEventListener('click',function(){
           const btnfilter = login.getAttribute("data-category")
            const compfilter = btnl.getAttribute("data-filter")
            console.log(btnfilter)
            console.log(compfilter)
           if (compfilter === btnfilter) {
            console.log(btnfilter)
           }
     ...
     /** * for(let i=0; i<totalbtns; i++){
        btns[i].addEventListener('click', function(){
            loginmodal.querySelector(".hide").classList.remove("hide")
            const filterBtnValue = this.getAttribute("data-filter")
            for(let k =0; k<totalforms; k++ ){
               ...
               /** if(validateFilter === login.getAttribute("data-category")){
                    
                    console.log(login.getAttribute("data-category"))
                }else{
                    login.classList.add("hide");
                    login.classList.remove("show")
                }
                if(validateFilter === signup.getAttribute("data-category")){
...
const closeLoginModel = (e)=>{
    if(e.target.classList.contains('login-modal')){
        loginmodel.style.display = 'None';
    }
}
theme.addEventListener('click', openThemeModel)
hireme.addEventListener("click", openLoginModel)
themeModel.addEventListener('click', closeThemeModel)
loginmodel.addEventListener('click', closeLoginModel)
/*===== FONTS =====*/
