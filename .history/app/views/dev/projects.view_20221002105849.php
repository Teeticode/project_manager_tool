            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-one active form-step">
                    ...
                    <div>
                        ...
                        <textarea  onclick="textclick()" style="<?= $errors['description']?  "border: 1px solid var(--color-danger); border-radius: 10px; padding:8px; margin-top:10px;" : " padding:8px; margin-top:10px;"?>" id="description" name="description" placeholder="brief description of application"><?= set_value('primary_subject')?></textarea>
                        <p id="sub_len" style="font-size:11px; font-weight:700;color:var(--color-dark-variant);"></p>
...
<script>
    ...
    prim_sub.addEventListener('keypress', (e)=>{
        ...
        var val = prim_sub.value.trim();
        document.querySelector('.js-error-description').style.display="None";
        if(trim(prim_sub.value.trim().length, ' ').length > 0 && prim_sub.value.length <= 50){
            sub_len.innerHTML = prim_sub.value.length;
            sub_len.style.color = '#ffbb55'
        }else if(prim_sub.value.trim().length > 50){
