            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-one active form-step">
                    ...
                    <div>
                        ...
                        <textarea  onclick="textclick()" style="<?= $errors['description']?  "border: 1px solid var(--color-danger); border-radius: 10px; padding:10px; margin-top:10px;" : " padding:10px; margin-top:10px;"?>" id="description" name="description" placeholder="brief description of application"><?= set_value('description')?></textarea>
                        <p id="sub_len" style="font-size:13px; font-weight:700;color:var(--color-dark-variant);"></p>
    ...
    <?php else:?>
        ...
        <div class="recent-orders-teeti" >
            ...
            <table >
                ...
                <br>
                <thead>
                    <h2 style="text-align: center; margin:9px;  color: var(--color-dark-variant);"> 
                        <?php if(!empty($rows)): ?>
                ...
                <tbody>
                    <?php foreach($rows as $row):?>
                        <tr>
                            ...
                            <td><?= esc($row->title)?></td>
                            <td class="remove-item"><?= esc($row->services_row->service ?? 'Unknown')?></td>
...
<script>
    ...
    var prim_sub = document.getElementById('description');
    var sub_len = document.getElementById('sub_len');
    
    function textclick(){
        prim_sub.disabled = false;
    }
    prim_sub.addEventListener('input', (e)=>{
        sub_len.innerHTML = (51 - (prim_sub.value.length))+ ' characters left';
        if(prim_sub.value.length == 0){
            sub_len.innerHTML = (51 - 1)+ ' characters left';
        }
        sub_len.style.color = '#ffbb55';
        document.querySelector('.js-error-description').style.display = "None" ??  "";
        
    })
    prim_sub.addEventListener('keypress', (e)=>{
        prim_sub.style.border ='None';
        console.log(prim_sub.value.length);
        sub_len.innerHTML = (51 - (prim_sub.value.length))+ ' characters left';
        if(prim_sub.value.length > 0 && prim_sub.value.length <= 50){
            sub_len.innerHTML = (51 - (prim_sub.value.length))+ ' characters left';
            sub_len.style.color = '#ffbb55'
        }else if(prim_sub.value.length >= 50){
            sub_len.innerHTML = (51 - (prim_sub.value.length))+ ' characters left';
            sub_len.style.color = '#ff7782';
            
            e.preventDefault();
            
        }else{
            sub_len.innerHTML = 50 - (prim_sub.value.length);
