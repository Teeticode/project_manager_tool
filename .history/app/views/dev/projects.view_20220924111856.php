...
<script>
    ...
    var sess_inp = sessionStorage.getItem("input");
    console.log(JSON.parse(sess_inp.value))
    if(sess_inp){
        var default_inp = document.querySelector('.default')
        default_inp.name=JSON.parse(sess_inp.name)
        default_inp.placeholder = JSON.parse(sess_inp.placeholder)
        default_inp.value = JSON.parse(sess_inp.value)
    }
