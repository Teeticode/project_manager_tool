            ...
            <form method="post" enctype="multipart/form-data">
                <div class="form-one active form-step">
                    ...
                    <div>
                        ...
                        <textarea onclick="textclick()" style="<?= $errors['primary_subject']?  "border: 1px solid var(--color-danger);" : ""?>" id="primary_subject" name="primary_subject" placeholder="brief description of application"><?= set_value('primary_subject')?></textarea>
                        <p id="sub_len" style="color:var(--color-success);">0</p>
                        <?php if(!empty($errors['primary_subject'])):?>

                            <small class="js-error-title text-danger "><?=$errors['primary_subject']?></small>
                        <?php endif;?>
                    </div>
                    <div>
...
<script>
    ...
    prim_sub.addEventListener('input', ()=>{
        if(prim_sub.value.length > 0 && prim_sub.value.length <= 20){
            sub_len.innerHTML = prim_sub.value.length;
            prim_sub.removeEventListener('keypress', ()=>{});
        }else if (prim_sub.value.length > 20) {
            sub_len.innerHTML = 20;
            sub_len.style.color = 'red';
            prim_sub.addEventListener('keypress', (e)=>{
                e.preventDefault();
            })
            
        }else{
            prim_sub.removeEventListener('keypress', (e)=>{e.defaultPrevented()});
            sub_len.innerHTML = prim_sub.value.length;
        }
